...
(comment
  ...
  (let [rows (->> "C:\\nikon\\projects\\stackoverflow-scripts\\76781_tweetable_hash\\tweetable-hash\\english.txt" slurp (re-seq #"[^\r\n]+"))]
...
(defn search [fname n-proc word-len seed]
  ...
  (let [f         #(reduce(fn[r i](mod(+(* r %)i)16777213))(map *(cycle(map int %2))(map int %3)))
        best-score (atom 1e6)
           ...
           (let [f (apply partial f p)
             ...
             (loop [score 0 seen-hashes {} rows rows]
               (if-let [row (first rows)]
                 (let [hash        (f row)
                       seen-count  (get seen-hashes hash 0)
